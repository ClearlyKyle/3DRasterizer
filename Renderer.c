    ...
    for (int y = aabb.minY; y <= aabb.maxY; y += 1)
    {
        ...
        // One step to the right
        {
            ...
            // if (mask.m128i_i32[3] && finalMask.m128i_i32[3])
            {
                ...
                const uint8_t red = (uint8_t)(pixelOffset[0] * light_thing[0]);
                const uint8_t gre = (uint8_t)(pixelOffset[1] * light_thing[1]);
                const uint8_t blu = (uint8_t)(pixelOffset[2] * light_thing[2]);
...
void Draw_Textured_Shaded_Triangle(const Rendering_data *render, const __m128 v0, const __m128 v1, const __m128 v2, const __m128 frag_colour)
{
    ...
    const __m128 v0_z = _mm_shuffle_ps(v0, v0, _MM_SHUFFLE(3, 3, 3, 3));
    const __m128 v1_z = _mm_shuffle_ps(v1, v1, _MM_SHUFFLE(3, 3, 3, 3));
    const __m128 v2_z = _mm_shuffle_ps(v2, v2, _MM_SHUFFLE(3, 3, 3, 3));
    ...
    for (int y = aabb.minY; y <= aabb.maxY + 1; y += 1)
    {
        ...
        for (int x = aabb.minX; x <= aabb.maxX + 1; x += 4,
                 w0 = _mm_add_epi32(w0, X_Step_w0),
                 w1 = _mm_add_epi32(w1, X_Step_w1),
                 w2 = _mm_add_epi32(w2, X_Step_w2))
        // One step to the right
...
void Draw_Textured_Smooth_Shaded_Triangle(const Rendering_data *render, const __m128 v0, const __m128 v1, const __m128 v2, __m128 nrm1, __m128 nrm2, __m128 nrm3, const __m128 frag_colour, const __m128 light_direction)
{
    ...
    const __m128 colour1 = _mm_mul_ps(Clamp_m128(_mm_add_ps(diffuse1, ambient), 0.0f, 1.0f), _mm_set1_ps(255.0f));
    const __m128 colour2 = _mm_mul_ps(Clamp_m128(_mm_add_ps(diffuse2, ambient), 0.0f, 1.0f), _mm_set1_ps(255.0f));
    const __m128 colour3 = _mm_mul_ps(Clamp_m128(_mm_add_ps(diffuse3, ambient), 0.0f, 1.0f), _mm_set1_ps(255.0f));

    // const __m128 colour1 = _mm_mul_ps(_mm_add_ps(diffuse1, ambient), _mm_set1_ps(255.0f));
    // const __m128 colour2 = _mm_mul_ps(_mm_add_ps(diffuse2, ambient), _mm_set1_ps(255.0f));
    // const __m128 colour3 = _mm_mul_ps(_mm_add_ps(diffuse3, ambient), _mm_set1_ps(255.0f));
    ...
    for (int y = aabb.minY; y <= aabb.maxY + 1; y += 1)
    {
        ...
        // One step to the right
        {
            ...
            if (finalMask.m128i_i32[3])
            {
                ...
                // const float dp_with_light = Calculate_Dot_Product_SIMD(normal, light_direction);
                // const float diff = (float)fmax((double)dp_with_light, 0.0);
                // const __m128 colour = _mm_mul_ps(_mm_mul_ps(light_colour, _mm_set1_ps(diff)), _mm_set1_ps(255.0f));
            ...
            if (finalMask.m128i_i32[2])
            {
                ...
                // const float diff = (float)fmax((double)Calculate_Dot_Product_SIMD(normal, light_direction), 0.0);
                // const __m128 colour = _mm_mul_ps(_mm_mul_ps(light_colour, _mm_set1_ps(diff)), _mm_set1_ps(255.0f));
...
void SIMD_OptimisedRasteriser(const Rendering_data *render, const __m128 v0, const __m128 v1, const __m128 v2, const __m128 frag_colour)
{
    ...
    //__m128i triArea = _mm_mullo_epi32(A0, v0_x);
    // triArea = _mm_add_epi32(triArea, _mm_mullo_epi32(B0, v0_y));
             ...
             sum2Row = _mm_add_epi32(sum2Row, bb2Inc))
    {
        ...
        // One step to the right
        {
            ...
            // Compute barycentric-interpolated depth
            //__m128 depth = _mm_mullo_epi32(w0, v0_z);
            // depth = _mm_add_epi32(depth, _mm_mullo_epi32(w1, v1_z));
            // depth = _mm_add_epi32(depth, _mm_mullo_epi32(w2, v2_z));
            ...
            // colour_at_pixel = (w0 * area * v1_colour) + (w1 * area * v2_colour) + (w2 * area * v3_colour);
            // const __m128 w0_and_area = _mm_mul_ps(_mm_mul_ps(w0, oneOverTriArea), _mm_cvtepi32_ps(colour_red_value));
            // const __m128 w1_and_area = _mm_mul_ps(_mm_mul_ps(w1, oneOverTriArea), _mm_cvtepi32_ps(colour_green_value));
            // const __m128 w2_and_area = _mm_mul_ps(_mm_mul_ps(w2, oneOverTriArea), _mm_cvtepi32_ps(colour_blue_value));

            // float colour[4];
            // const __m128 final_colour_values = _mm_add_ps(_mm_add_ps(w0_and_area, w1_and_area), w2_and_area);
            //_mm_store_ps(colour, final_colour_values);

            // const RGB draw_colour_values = {(unsigned char)colour[0], (unsigned char)colour[1], (unsigned char)colour[2]};
